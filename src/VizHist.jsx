  ...
  loadCountry(db, team, other_team) {
    ...
    db.ref(`summary/${team}`).once('value', snapshot => {
      snapshot.forEach(function (childSnapshot) {
        ...
        const matches = childSnapshot.val()
        matches.map((match) => {
          if (other_team) {
    ...
    Object.keys(results).map((key) => {
      ...
      results[key].map((match) => {
        if (match.home_team === team) {
          if (match.home_score > match.away_score) stat.v++
          else if (match.home_score < match.away_score) stat.l++
          else stat.t++
        } else {
          if (match.home_score > match.away_score) stat.l++
          else if (match.home_score < match.away_score) stat.v++
          else stat.t++
        }
      })
      tab[key] = stat
  ...
  render() {
    return (
      <div style={{ paddingTop: "20px", paddingBottom: "300px", paddingLeft: "10%", paddingRight: "10%", width: "80%", position: "relative", backgroundColor: "black" }}>
        ...
        <MatchViz homeTeam="Brazil" awayTeam="Germany" games={this.state.cross} darkMode={true} />
