      ...
      <div ref={this.ref} style={{ margin: "auto", maxWidth: "500px", position: "relative", textAlign: "center" }}>
        <svg viewBox="0 0 180 180">
          <g transform="translate(90,160) rotate(225 0 0)">
            ...
            {[...Array(6).keys()].map((gols) => (
              <g key={gols}>
                <line x1={gols * unit} y1={0} x2={gols * unit} y2={100} style={{ stroke: color, strokeWidth: 0.2 }} opacity={0.2} />
                <line x1={0} y1={gols * unit} x2={100} y2={gols * unit} style={{ stroke: color, strokeWidth: 0.2 }} opacity={0.2} />
                {gols < 6 ? (
                  <g>
                    {gols > 0 && <text opacity={0.5} fill={color} transform={`rotate(180 ${gols * unit} 100)`} style={{ fontSize: "4px", fontFamily: "Lato" }} x={gols * unit} dx={-4} dy={-2} y={200}>{gols}</text>}
            ...
            <line x1={0} y1={0} x2={100} y2={0} style={{ stroke: color, strokeWidth: 0.6 }} opacity={0.8} />
            ...
            {Object.keys(this.state.summary).map((key, i) => {
              ...
              if (a !== h) {
                const circleColor = a > h ? pink500 : blue500
                return <circle key={i} fill={circleColor} cx={a * unit} cy={h * unit} r={r} opacity={0.8} />
              } else {
                ...
                {(nodes) => (
                  <g>
                    {nodes.map(({ key, data, state }) => {
                      const { opacity, opacity2, x, y } = state
                      return (
                        <g key={key}>
                          <circle
        ...
        {this.state.games && this.state.games.length > 0 && (
          <span
            ...
            style={{ fontSize: "12px", color: "#999", cursor: "pointer", fontFamily: "Lato", textDecoration: "underline", userSelect: "none", marginBottom: "8px", display: "inline-block", letterSpacing: "2px" }}
          >animate</span>
        )}
      </div>
    )
